'''
This script contains the pipelines for the experiments in the paper
'''
...
def pipe_corr_att_or_unatt(Subj_ID, eeg_multisubj_list, feat_att_list, feat_unatt_list, fs, L_EEG, L_Stim, offset_EEG, offset_Stim, TRAIN_WITH_ATT, eeg_ori_list=None, dim_list_EEG=None, dim_list_Stim=None, n_components=3, saccade_multisubj_list=None, V_eeg=None, V_Stim=None, PLOT=False, figure_dir=None, SAVERES=False, table_dir=None, OVERWRITE=False, feat_name='ObjFlow', COMBINE_ATT_UNATT=False, REGFEATS=False):
    '''
    ...
    COMBINE_ATT_UNATT: Feed the attended and unattended features into the CCA together
    REGFEATS: When calculating the correlations in the test set, set REGFEATS to True if control for the competing features (Corr -> Partial Corr)
    '''
    ...
    if saccade_multisubj_list is not None:
        ...
        mask_list = None
    CCA = algo.CanonicalCorrelationAnalysis(eeg_onesubj_list, feat_att_list, fs, L_EEG, L_Stim, offset_EEG, offset_Stim, dim_list_EEG=dim_list_EEG, dim_list_Stim=dim_list_Stim, n_components=n_components, REGFEATS=REGFEATS, mask_list=mask_list)
    ...
    if SAVERES:
        table_name = table_dir + f'{feat_name}_Corr_Train_{train_type}_Mask_{ifmask}_REG_{REGFEATS}.csv'
...
def pipe_vad(Subj_ID, eeg_multisubj_list, feat_att_list, feat_unatt_list, fs, L_EEG, L_Stim, offset_EEG, offset_Stim, trial_len_list, table_dir, dim_list_EEG=None, dim_list_Stim=None, saccade_multisubj_list=None, BOOTSTRAP=True, V_eeg=None, V_Stim=None, n_components=3, nb_comp_into_account=2, signifi_level=False, message=True, OVERWRITE=False, feat_name='ObjFlow', COMBINE_ATT_UNATT=False, REGFEATS=False):
    ...
    dim_list_Stim: Similar to dim_list_EEG, but for the dimensions of the stimulus features.
    saccade_multisubj_list: Saccade data. A mask will be created if saccade data is provided to exclude the time points around saccades.
    BOOTSTRAP: True if selecting trials with given length randomly (with overlap), False if dividing the trials without overlap
    V_eeg, V_Stim: If want to use pretrained filters trained from single object data, provide the filters here.
    nb_comp_into_account: Number of components into account when making the decision
    OVERWRITE: True if want to overwrite the existing results in the table
    COMBINE_ATT_UNATT: Feed the attended and unattended features into the CCA together
    '''
    res = []
    eeg_onesubj_list = [eeg[:,:,Subj_ID] for eeg in eeg_multisubj_list]
...
def pipe_single_obj(Subj_ID, eeg_multisubj_list, feat_list, fs, L_EEG, L_Stim, offset_EEG, offset_Stim, figure_dir, table_dir, dim_list_EEG=None, dim_list_Stim=None, saccade_multisubj_list=None, n_components=3, PLOT=False, OVERWRITE=False, feat_name='ObjFlow', CV=False):
    '''
    ...
    If CV is True, cross-validation is performed on single-object dataset. Otherwise, the single-object data is only used to train the filters to be used in the overlaid-object data.
    '''
    eeg_onesubj_list = [eeg[:,:,Subj_ID] for eeg in eeg_multisubj_list]
...
def pipe_saccade(nested_data, nested_aug_data, fs, L_EEG, L_Stim, offset_EEG, offset_Stim, trial_len_list=None, BOOTSTRAP=True, n_components=3, nb_comp_into_account=2, table_dir=None, OVERWRITE=False, SYNMASK=False, REGFEATS=False, feat_name='ObjFlow'):
...
##############################################################################
# Pipelines for the experiments that are not included in the paper
# Therefore the functions are not actively maintained and tested
##############################################################################
